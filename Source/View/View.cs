...
//						WindowHelper.Selector ("Select", realList, toString, t => CurrentlySelected = t);
	...
	{
		private readonly GUIContent label;
		private readonly Hyperedit.TryParse<T> parser;
		private readonly Func<T, string> toString;
		...
		public TextBoxView(string label, string help, T start, Hyperedit.TryParse<T> parser, Func<T, string> toString = null, Action<T> onSet = null)
		{
		...
		public void Draw()
		{
			...
			if (label != null || onSet != null)
			{
				if (onSet != null && Valid && GUILayout.Button(Localizer.GetStringByTag("#KRASH_Button_Set"))) // "Set"
				{
					onSet(Object);
					Hyperedit.Extensions.ClearGuiFocus();
				}
	...
	public class TabView : IView
	{
		...
		public void Draw()
		{
			...
			foreach (var view in views)
			{
				...
				{
					GUILayout.Button(view.Key, Hyperedit.Extensions.PressedButton);
				}
				else
				{
